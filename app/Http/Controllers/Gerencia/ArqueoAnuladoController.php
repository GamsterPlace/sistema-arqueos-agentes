...
class ArqueoAnuladoController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $extemporaneo = trim(
            (string) $request->string('extemporaneo')
        );
        ...
        $arqueos = DB::table('arqueos as arq')
            ...
            ->when(
                $extemporaneo !== '',
                function ($query) use ($extemporaneo): void {
                    if ($extemporaneo === 'SI') {
                        $query->where(
        ...
        $totalAnulados = DB::table('arqueos')
            ->where('estado', 'ANULADO')
            ->count();

        $anuladosAgente = DB::table('arqueos')
            ->where('estado', 'ANULADO')
            ->where('tipo', 'DIARIO_AGENTE')
            ->count();

        $anuladosPromotor = DB::table('arqueos')
            ->where('estado', 'ANULADO')
            ->where('tipo', 'VISITA_PROMOTOR')
            ->count();

        $anuladosExtemporaneos = DB::table('arqueos')
            ->where('estado', 'ANULADO')
