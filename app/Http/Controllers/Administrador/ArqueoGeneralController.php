...
class ArqueoGeneralController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $rutas = DB::table('rutas as r')
            ...
            ->orderBy('r.nombre')
            ->get([
                'r.id','r.codigo','r.nombre','r.region_id',
        ...
        $arqueos = DB::table('arqueos as arq')
            ...
            ->when($extemporaneo !== '', function($q) use ($extemporaneo) {
                if ($extemporaneo === 'SI') $q->where('arq.fuera_fecha_ordinaria',true);
                if ($extemporaneo === 'NO') $q->where('arq.fuera_fecha_ordinaria',false);
            })
            ...
            ->select([
                ...
                'arq.estado','arq.fuera_fecha_ordinaria','arq.total_arqueado',
                'arq.saldo_sistema','arq.diferencia','a.codigo_agente',
                'a.nombre_negocio','r.codigo as ruta_codigo',
        ...
        $resumen = (object)[
            ...
            'agente' => DB::table('arqueos')->where('tipo','DIARIO_AGENTE')->count(),
            'promotor' => DB::table('arqueos')->where('tipo','VISITA_PROMOTOR')->count(),
            'certificados' => DB::table('arqueos')->where('estado','CERTIFICADO')->count(),
            'pendientes' => DB::table('arqueos')->where('estado','PENDIENTE_CERTIFICACION')->count(),
            'anulados' => DB::table('arqueos')->where('estado','ANULADO')->count(),
            'extemporaneos' => DB::table('arqueos')->where('fuera_fecha_ordinaria',true)->count(),
        ];

        return view('administrador.arqueos.index', compact(
            'arqueos','agentes','promotores','regiones','rutas','agenteId',
            'promotorId','regionId','rutaId','tipo','estado','buscar','desde',
            'hasta','extemporaneo','resumen'
        ));
    }
    ...
    public function show(Request $request, Arqueo $arqueo): View
    {
        ...
        return view('administrador.arqueos.show', compact(
            'arqueo','billetes','monedas','responsable'
        ));
    }
    ...
    public function imprimir(Request $request, Arqueo $arqueo): Response
    {
        ...
        $pdf = Pdf::loadView(
            'administrador.arqueos.pdf',
            compact('arqueo','billetes','monedas','responsable')
