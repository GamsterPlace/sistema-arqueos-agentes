...
<head>
    ...
    @php
        ...
        if (! file_exists($rutaLogoAgentesMicoope)) {
            throw new \RuntimeException(
                'No se encontró el logo de Agentes MICOOPE en: ' . $rutaLogoAgentesMicoope
            );
        }
        ...
        $logoAgentesMicoope = 'data:image/png;base64,' . base64_encode(
            file_get_contents($rutaLogoAgentesMicoope)
        );
    @endphp
...
<div class="page">
    ...
    @php
        $textoCertificacion = wordwrap(
            trim((string) $arqueo->certificacion),
            118,
            "\n",
            false
        );

        $lineasCertificacion = array_slice(
            explode("\n", $textoCertificacion),
            0,
            3
        );

        while (count($lineasCertificacion) < 3) {
            $lineasCertificacion[] = '';
        }
    @endphp
    ...
    <div class="certification">
        ...
        @foreach ($lineasCertificacion as $linea)
        ...
        <div class="signature signature-promoter">
            <div class="electronic-signature">
                @if ($firmaCertificador)
                    ...
                    <div class="electronic-signature-code">
                        Código: {{ strtoupper(substr($firmaCertificador->firma_electronica, 0, 20)) }}
                    </div>
                @else
                    <div class="signature-pending">
