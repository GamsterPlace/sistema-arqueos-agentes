...
@push('styles')
<style>
    .promotor-detail-page{max-width:1280px;margin:0 auto}
    ...
    .detail-heading-icon svg,.btn-detail svg,.panel-icon svg{fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}
    .detail-heading-icon svg{width:26px;height:26px}
    .detail-heading h2{margin:0 0 4px;color:var(--azul-profundo);font-size:25px;line-height:1.15;overflow-wrap:anywhere}
    ...
    .document-business{margin:0 0 6px;color:var(--azul-profundo);font-size:20px;line-height:1.25}
    ...
    .status-badge{display:inline-flex;align-items:center;gap:7px;min-height:32px;padding:0 11px;border-radius:999px;font-size:11px;font-weight:800;letter-spacing:.025em;text-transform:uppercase;white-space:nowrap}
    .status-badge:before{content:"";width:7px;height:7px;border-radius:50%;background:currentColor}
    .status-certified{color:#08783d;background:#e9f8f0;border:1px solid #c7ecd8}
    .status-pending{color:#986b00;background:#fff8e3;border:1px solid #f2e2a8}
    .status-cancelled{color:#b33a3a;background:#fff0f0;border:1px solid #f2cccc}
    ...
    .panel-icon.result{color:var(--verde-oscuro);background:#eaf8f1}
    ...
    .info-row{display:grid;grid-template-columns:155px minmax(0,1fr);gap:15px;align-items:center;min-height:52px;border-bottom:1px solid #edf1f4}
    ...
    .money-summary{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;padding:18px 18px 5px}
    .money-card{padding:14px;border:1px solid #e2eaf0;border-radius:13px;background:var(--fondo-suave)}
    .money-card .label{display:block;margin-bottom:5px;color:var(--texto-secundario);font-size:11px;font-weight:800;letter-spacing:.035em;text-transform:uppercase}
    .money-card .value{display:block;color:var(--azul-profundo);font-size:18px;font-weight:800;line-height:1.2}
    .difference-card{margin:7px 18px 18px;padding:15px 16px;display:flex;align-items:center;justify-content:space-between;gap:14px;border:1px solid #cfe0eb;border-radius:13px;background:#f2f7fb}
    .difference-card .label{color:#527087;font-size:12px;font-weight:800;letter-spacing:.035em;text-transform:uppercase}
    .difference-card .value{color:var(--azul-principal);font-size:20px;font-weight:800;white-space:nowrap}
    .text-block{margin:0 18px 14px;padding:14px 15px;border:1px solid #e3eaf0;border-radius:13px;background:#fff}
    .text-block:last-child{margin-bottom:18px}
    .text-block-title{margin-bottom:7px;color:var(--texto-secundario);font-size:11px;font-weight:800;letter-spacing:.035em;text-transform:uppercase}
    .text-block p{margin:0;color:var(--texto);font-size:13px;line-height:1.6;white-space:pre-line}
    @media(max-width:950px){.detail-grid{grid-template-columns:1fr}}
    @media(max-width:700px){.detail-header,.document-banner{align-items:flex-start;flex-direction:column}.header-actions{width:100%}.btn-detail{flex:1}.money-summary{grid-template-columns:1fr}.info-row{grid-template-columns:1fr;gap:4px;padding:12px 0}.info-value{text-align:left}.difference-card{align-items:flex-start;flex-direction:column}}
</style>
@endpush
...
@php
    $a = $arqueo->agente;
    $p = trim(($promotor->nombres ?? '') . ' ' . ($promotor->apellidos ?? ''));
...
<div class="promotor-detail-page">
    <div class="detail-header">
        <div class="detail-heading">
            ...
            <div>
                ...
                <p>Detalle completo del arqueo realizado durante la visita del Promotor.</p>
            </div>
        </div>

        <div class="header-actions">
            <a href="{{ route('administrador.arqueos-promotores.index') }}" class="btn-detail">
            ...
            <a
                target="_blank"
                href="{{ route('administrador.arqueos-promotores.imprimir', $arqueo->id) }}"
    ...
    <section class="document-banner">
        <div>
            <div class="document-eyebrow">Visita de Promotor</div>
            <h3 class="document-business">
                {{ $a?->nombre_negocio ?? $arqueo->nombre_negocio_historico ?? 'Agente MICOOPE' }}
            </h3>

            <div class="document-meta">
                <span>Promotor: <strong>{{ $nombrePromotor }}</strong></span>
                <span>Agente: <strong>{{ $a?->codigo_agente ?? $arqueo->codigo_agente_historico ?? '—' }}</strong></span>
        ...
        <section class="detail-panel">
            <div class="panel-heading">
                ...
                <div>
                    ...
                    <p>Datos de la visita, Promotor y Agente MICOOPE.</p>
                </div>
            </div>
            ...
            <div class="info-list">
                <div class="info-row">
                    ...
                    <span class="info-value primary">{{ $nombrePromotor }}</span>
                </div>
        ...
        <section class="detail-panel">
            <div class="panel-heading">
                ...
                <div>
                    ...
                    <p>Resumen de los valores registrados por el Promotor.</p>
                </div>
            </div>
            ...
            <div class="money-summary">
                <div class="money-card">
                    ...
                    <span class="value">Q {{ number_format((float) $arqueo->saldo_sistema, 2) }}</span>
                </div>

                <div class="money-card">
                    <span class="label">Total Arqueado</span>
                    <span class="value">Q {{ number_format((float) $arqueo->total_arqueado, 2) }}</span>
                </div>
            </div>

            <div class="difference-card">
                <span class="label">Diferencia</span>
                <span class="value">Q {{ number_format(abs((float) $arqueo->diferencia), 2) }}</span>
            </div>

            <div class="text-block">
                <div class="text-block-title">Observaciones</div>
                <p>{{ $arqueo->observaciones ?: 'Sin observaciones registradas.' }}</p>
            </div>

            <div class="text-block">
                <div class="text-block-title">Calificación</div>
                <p>{{ $arqueo->certificacion ?: 'Sin calificación registrada.' }}</p>
            </div>
        </section>
    </div>
</div>
